...
export default function SkillProgress() {
  ...
  const fetchUsers = () => {
    axios.get("/api/Users/getChildren")
      .then(response => {
  ...
  const fetchFinishedSkills = (User_ID) => {
    axios.get(`/api/SkillProgress/getFinishedSkillsForUser/${User_ID}`)
      .then(response => {
  ...
  const fetchUserSkills = () => {
    axios.get("/api/SkillProgress/getAllSkillProgress")
      .then(response => {
  ...
  const handleStatusChange = (User_ID, Skill_ID, newStatus) => {
    ...
    axios.put("/api/SkillProgress/updateSkillProgressStatus", requestData)
      .then(response => {
  ...
  const handleFeedbackChange = (User_ID, Skill_ID, newFeedback) => {
    axios.put(`/api/SkillProgress/updateUserSkillFeedback?User_ID=${User_ID}&Skill_ID=${Skill_ID}&feedback=${newFeedback}`)
      .then(response => {
        alert(response.data.statusMessage);
  ...
  const handleNewSkillSubmit = () => {
    if (selectedUser_ID && newSkill_ID) {
      axios.post(`/api/SkillProgress/addSkillProgress?User_ID=${selectedUser_ID}&Skill_ID=${newSkill_ID}`)
        .then(response => {
          alert(response.data.statusMessage);
          fetchUserSkills();
          setNewSkill_ID('');
